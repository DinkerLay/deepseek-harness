...
Permanent Session deletion is implemented separately by the [Host deletion plugin decision](../../implemented/architecture/2026-08-20-dsh-session-deletion-plugin.md). This note retains the storage and Workspace rationale that predates that capability.

The later [Workspace registration deletion decision](../../implemented/feature/2026-07-27-workspace-registration-deletion.md) supersedes only that coupling: deleting a Workspace registration preserves its Sessions and their logs, while Session deletion remains a separate operation. The cascade design below is therefore not the Workspace GUI delete semantic.
...
| `SessionPersistence.delete` extension + cascade orchestration | `packages/session/session-persistence*` | new method on the existing seam | ✓ separate implemented decision |
...
- **Session deletion remains separate from Workspace deletion.** The later [Workspace registration deletion decision](../../implemented/feature/2026-07-27-workspace-registration-deletion.md) ships `ctx.workspaceRegistry.delete(id)` as a metadata-only operation that preserves Sessions and logs. The [Host deletion plugin decision](../../implemented/architecture/2026-08-20-dsh-session-deletion-plugin.md) owns recursive Session deletion, running checks, and retry convergence.
...
| session delete contract | absent-id retry, deleted-id reuse, unmaterialized intent, serialization with in-flight appends, the deleted event | jsonl, sqlite |
