...
永久 Session 删除由 [Host deletion plugin 决策](../../implemented/architecture/2026-08-20-dsh-session-deletion-plugin.zh.md)单独实现。本文保留早于该 capability 的 storage 与 Workspace rationale。

后续的 [Workspace 注册记录删除决策](../../implemented/feature/2026-07-27-workspace-registration-deletion.zh.md)取代的仅是上述耦合关系：删除 Workspace 注册记录会保留相关 Session 及其日志，Session 删除仍是独立操作。因此，下文的级联设计并不是 Workspace GUI 的删除语义。
...
| `SessionPersistence.delete` 扩面 + 级联删编排 | `packages/session/*` | 既有 seam 新方法 | ✓ 独立 implemented decision |
...
依赖方向：`dsh-workspace` → `dsh-domain` → `dsh-storage` ← 两后端。`dsh-workspace` 另依赖 `ctx.sessionPersistence` 的只读面（attach 的 cwd 校验读 session header；服务缺席时 attach 直接拒绝——无法校验即不写账）。独立 deletion package 持有 Session reservation、Agent idle disposal 与 persistence cascade behavior。
...
- **Session 删除继续与 Workspace 删除分离。** 后续的 [Workspace 注册记录删除决策](../../implemented/feature/2026-07-27-workspace-registration-deletion.zh.md)已将 `ctx.workspaceRegistry.delete(id)` 作为仅删除元数据、保留 Session 与日志的操作交付。[Host deletion plugin 决策](../../implemented/architecture/2026-08-20-dsh-session-deletion-plugin.zh.md)持有递归 Session 删除、运行中检查与 retry convergence。
...
| session delete 约定 | absent-id retry、已删 id 复用、未物化 intent、与在途 append 串行、deleted 事件 | jsonl、sqlite |
