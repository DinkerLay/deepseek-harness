...
Core tests cover restore, own-session attribution, invalid paths and post-commit observation. Real filesystem and AgentLoop shell tests verify write/read consistency without changing shared files. The [filesystem regression](../../../../packages/fs/tool-fs/tests/integration.spec.ts) verifies one Session reading shared input and then editing and restoring its recorded execution directory. Profile-level snapshot coverage is tracked by the release-integration plan.
