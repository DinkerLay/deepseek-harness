...
核心测试覆盖恢复、自身 Session 归属、无效路径和提交后观察。真实文件系统与 AgentLoop Shell 测试验证读写一致且共享文件不变。[Loader 快照](../../../../examples/headless-agent/tests/execution-directory.snapshot.ts)记录一个 Session 读取共享输入，然后在记录的执行目录使用真实文件、搜索和 Shell 工具。
