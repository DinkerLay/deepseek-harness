...
The [stopped-parent continuation regression](../../../../packages/subagent/subagent/tests/continuation.spec.ts) installs an explicit delivery policy and pins the absence of another parent model request. Profile-level snapshot validation remains separate from these package regressions.

The [same-route title regression](../../../../packages/session/session-title/tests/provider.spec.ts) pins automatic updates across two Turns sharing one request header. The title service checks that the marked main request contains the pending human message; it does not assume that user/message precedes step/start.
