...
首消息提供方仅在新会话首次创建回退标题时调度一次。自动执行失败后，后续提示词不会重新调度；`refresh()` 是重试路径。全部消息提供方会在每条合格且由人类发出的提示词后调度，并传入截至该修订的所有合格消息，不包含 fork 继承的种子历史。较新的修订会中止并取代更早的待执行或活跃工作。
...
`rename(session, title)` 同步接受用户标题：按已接受标题的字节上限规范化文本、取代在途自动工作，并追加一条第三种来源 `user` 的 `session/title` 事件。最新标题来源为 user 即钉住该会话：只要它还在，`onUserMessage` 在任一节奏下都不再安排自动修订。显式 `refresh()` 通过 `session/title-policy` 解除锁定，并在可生成替代标题之前保留当前可用标题。Web host 将其暴露为 `session.rename` unary 方法（冷会话先恢复），并返回规范化后的标题及其事件 seq，使 client 在推送帧到达前就结算自己的 `title` 投影格。

### Fork 与消费方

Fork 继承临时标题，但自动命名只使用自己的种子后输入；继承的用户锁定不会锁定子会话。[Session 创建准备决策](../architecture/2026-09-07-session-provisioning-and-delivery-attribution.zh.md) 定义该分支命名策略。
