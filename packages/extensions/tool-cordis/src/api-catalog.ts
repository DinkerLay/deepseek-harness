...
export const SERVICE_API: readonly ServiceApiEntry[] = [
  ...
  {
    key: 'clientModules',
    summary: 'Host-owned browser factory inventory, activation graph and content-addressed bundle routes.',
    description: 'Host-owned browser factory inventory, activation graph and content-addressed bundle routes.',
    methods: [
      {
  ...
  },
  {
    ...
    methods: [
      ...
      {
        signature: 'readonly supportsMigrationCoordinates: boolean = false',
        description: 'Whether this provider can recover mappings from retained historical generations.',
        parameters: [],
      },
      {
      ...
      },
      {
        ...
        description: 'Permanently remove one stored Session identity. Providers serialize this operation with write ownership and remove every generation they own. The default rejects so callers can distinguish unsupported storage before reserving live lifecycle state.',
        parameters: [{ name: '_id', description: 'stored Session identity to remove.' }, { name: '_options', description: 'optional cancellation observed before ownership.' }],
        returns: 'the removed header, or `undefined` when the identity is absent.',
      },
      {
        signature: 'async listDeletionHeaders(options?: SessionPersistenceListOptions): Promise<readonly SessionHeader[]>',
        description: 'List headers used for recursive deletion discovery. Implementations with additional in-process lazy identities may override this method.',
        parameters: [{ name: 'options', description: 'optional cancellation.' }],
        returns: 'one detached header per currently known identity.',
      },
      {
        signature: 'migrationCoordinates( _id: SessionId, _options?: SessionPersistenceOpenOptions, ): Promise<SessionMigrationCoordinates | undefined>',
        description: 'Read the exact mapping from a retained historical generation to the current logical generation without publishing or rewriting either one. Implementations return the same mapping after successor publication while the source generation remains available, allowing interrupted sidecar migrations to resume idempotently.',
        parameters: [{ name: '_id', description: 'Session identity whose retained source generation is inspected.' }, { name: '_options', description: 'optional cancellation for read-only preparation.' }],
  ...
  },
  {
    ...
    methods: [
      ...
      {
        signature: 'readonly sourcePlugin: string',
        description: 'Package identity attached by the loop to this provider\'s new messages.',
        parameters: [],
      },
      {
        signature: 'readonly legacySourcePlugins: readonly string[]',
        description: 'Historical package identities accepted during runtime-context restoration.',
        parameters: [],
      },
      {
...
export const TYPE_API: readonly TypeApiEntry[] = [
  ...
  {
    name: 'SessionMigrationCoordinates',
    declaration: 'export interface SessionMigrationCoordinates {\n    readonly sessionId: SessionId;\n    readonly source: SessionMigrationGeneration;\n    readonly target: Omit<SessionMigrationGeneration, \'revision\'> & {\n        readonly revision?: SessionPersistenceRevision;\n    };\n    readonly targetSeqBySourceSeq: readonly (SessionSeq | null)[];\n    readonly targetEventCount: SessionLogOffset;\n}',
  },
  {
    name: 'SessionMigrationGeneration',
    declaration: 'export interface SessionMigrationGeneration {\n    readonly version: number;\n    readonly revision: SessionPersistenceRevision;\n}',
  },
  {
