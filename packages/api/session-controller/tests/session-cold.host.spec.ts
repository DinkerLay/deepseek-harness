 ...
 * bounded cold listing, Agent-free history reads, subagent ownership
...
describe('sessions.list cold merge', () => {
  it('publishes and lists a live Session under its effective execution directory', async () => {
    const ctx = new Context()
    await ctx.plugin(SessionStore)
    await ctx.plugin(AgentRegistry)
    const summaries: Array<{ sessionId: SessionId; cwd?: string }> = []
    ctx.on('api-session/added', (summary) => { summaries.push(summary) })
    const remote = createSessionTestRemote(ctx, {
      defaultModelSelection: () => ({ provider: 'p', model: 'm' }),
      cwd: '/tmp',
    })
    const session = ctx.sessions.create(sid('effective-cwd'), { meta: { cwd: '/shared' } })
    session.append('session/execution-directory', { sessionId: session.id, cwd: '/branches/effective' })

    expect(summaries.at(-1)).toMatchObject({ sessionId: session.id, cwd: '/branches/effective' })
    await expect(remote.list(request({}))).resolves.toMatchObject({
      ok: true,
      value: { items: [expect.objectContaining({ sessionId: session.id, cwd: '/branches/effective' })] },
    })
