...
Clients can create, resume, fork, list, inspect, interrupt, and delete Sessions through the Host gateway. The same API exposes model choices, invocable skills, file references, and workspace opening. Exact forks accept a reserved Session id and directory, reuse only matching destinations, and reject open-Turn boundaries. Deletion checks source ownership before publishing a replacement and reports committed removals to connected clients.
