...
/** Effective file resolution context for one Session identity. */
export interface WorkspaceFileScope {
  /** Session identity received on the wire. */
  readonly sessionId: SessionId
  /** Current execution root, or the deployment fallback when the Session has no cwd. */
  readonly workspaceRoot: string
}

declare module '@deepseek-ai/dsh-typert-protocol' {
  interface TypertLookupMap {
    /** Resolve a Session id to its effective execution root without activating an Agent. */
...
export class WorkspaceFiles extends TypertRemoteService {
  static inject = ['fs', 'sandboxPolicy', 'sessions', 'sessionQuery', 'typert']
  ...
  constructor(ctx: Context, private readonly config: Config) {
    ...
    ctx.inject(['sessions', 'sessionQuery', 'typert'], (scope) => {
      scope.typert.lookups.register('workspaceFileScope', {
        ...
        resolve: async (sessionId) => {
          const live = scope.sessions.get(sessionId)
          if (live !== undefined) {
            return {
              sessionId,
              workspaceRoot: resolveSessionCwd(live) ?? scope.sandboxPolicy.workspaceRoot,
            }
          }
          let observation
          try {
            observation = await scope.sessionQuery.observeSession(sessionId)
          } catch (error: unknown) {
            if (error instanceof SessionQueryError && error.code === 'SESSION_QUERY_SESSION_NOT_FOUND') return undefined
            throw error
          }
          using stored = observation
          return {
            sessionId,
            workspaceRoot: executionDirectoryFromEvents(stored.header, stored.events)
              ?? scope.sandboxPolicy.workspaceRoot,
          }
        },
      })
    })
