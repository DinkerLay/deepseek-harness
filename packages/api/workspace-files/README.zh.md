...
把本包与 `dsh-fs`、`dsh-sandbox-policy`、Session store、Session Query 和 Typert Gateway 一起挂载；bundle 把它紧随 Session Controller 之后挂载。每个方法都在线路上携带 Session 身份，Client 调用 `remote.workspaceFiles.read(sessionId, path, range, signal)`、`stat(sessionId, path, signal)`、`readBytes(sessionId, path, range, signal)`、`list(sessionId, path, signal)` 或 `changes(sessionId, signal)`，从不自己指定根。Host 对 live Session 使用有效执行目录，对 cold Session 使用一次观察；它不会激活 Agent 或借用父 Session 的根。Session 缺失时返回 `gateway/lookup-not-found`。
...
`read`、`readBytes`、`readAll`、`readRelated` 与 `stat` 接受绝对路径或相对于所选 Session 有效执行根的路径。组合文件系统决定路径是否可读；本服务不额外要求文件读取限定于该根。`readRelated` 从基文件所在目录解析相对文件系统路径，基文件或目标文件位于根外时同样适用。这些方法以文件系统执行环境中的绝对路径报告文件。`list` 仍限定于执行根，并以相对于该根的路径报告被列举目录。`changes` 同样只报告执行根内已埋点的文件系统观察。
...
`changes` 是 `stream` 模式的 Remote。一代流注册观察队列并解析 Session 执行根之后，才产出 `{ kind: 'ready' }`。随后产出 `{ kind: 'change', change }`，其中 `change` 对存在的文件为 `{ absolutePath, version }`，对被观察到已消失的文件为 `{ absolutePath, absent: true }`。来源是按该根内目标过滤的 `fs/observed`；操作系统并未被监视。一代流首次拉取后的观察都会排队，包括解析根期间的观察。流在取消或插件释放时结束。
...
经 `ctx.fs` 的读取使用后端的读取权限；沙箱后端限制写与编辑，而不限制读取。Typert lookup 对 live Session 从 `Session.executionDirectory` 导出 `WorkspaceFileScope`，对 cold Session 则通过 Session Query 一次观察调用 `executionDirectoryFromEvents()`。本服务增加普通文件检查与有界传输，根包含要求只属于目录列举与变更观察。页从 `streamText` 切出，后者逐块解码并拒绝非 UTF-8：切页器对窗口之前的行只计数不保留，对窗口内的每个片段先按字节上限验收再缓冲，并在窗口之后的第一个字符处返回。流之前的一次 `stat` 给出页所报告的版本与大小。
