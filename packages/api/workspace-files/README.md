...
Mount the package beside `dsh-fs`, `dsh-sandbox-policy`, the Session store, Session Query, and the Typert Gateway; the bundle does so right after the Session Controller. Every method takes the Session identity on the wire, so a Client calls `remote.workspaceFiles.read(sessionId, path, range, signal)`, `stat(sessionId, path, signal)`, `readBytes(sessionId, path, range, signal)`, `list(sessionId, path, signal)`, or `changes(sessionId, signal)` and never names a root itself. The Host uses the effective execution directory from a live Session or one cold Session observation; it does not activate an Agent or borrow a parent Session's root. A missing Session returns `gateway/lookup-not-found`.
...
`read`, `readBytes`, `readAll`, `readRelated`, and `stat` accept an absolute path or one relative to the selected Session's effective execution root. The composed filesystem decides whether the path is readable; the service does not impose root containment on file reads. `readRelated` resolves a relative filesystem path from the base file's directory, including when either file is outside the root. These methods report the file's absolute path in the filesystem's execution world. `list` remains execution-root-scoped and reports the listed directory relative to that root. `changes` likewise reports only instrumented filesystem observations inside the execution root.
...
Reads through `ctx.fs` use the backend's read authority; the sandboxing backend fences writes and edits, not reads. A Typert lookup derives `WorkspaceFileScope` from `Session.executionDirectory` for a live Session or `executionDirectoryFromEvents()` over one cold Session Query observation. The service adds regular-file checks and bounded transfer, while root containment belongs only to directory listing and change observation. A page is cut from `streamText`, which decodes and rejects non-UTF-8 chunk by chunk: the cutter counts lines before the window without keeping them, admits each in-window segment against the byte cap before buffering it, and returns at the first character past the window. One `stat` before the stream names the version and size the page reports.
