...
The Client export registers its locale dictionaries and one conversation-header slot with four optional child slots for member metadata, task-card action, task-heading action, and graph through Cordis effects; it mounts no Remote namespace. Disposing the plugin fiber removes the registrations.
