---
description: "十四个让模型创建、退队、发消息、监看与协调 teammate 的工具，供挂载实验性 Team 插件的组合使用。"
...
本包让模型创建具名 teammate、让成员退队、发送消息、检查 Lead 可见的私聊、等待进展、中断卡住的工作，并通过共享任务板协调。每个团队成员都会获得相同的十四个工具，以及在共享工作区协调的指引。当模型只应在你明确要求后运行团队时，选择本包。它会取代同名的旧版 subagent 控件，因此同时需要两者的组合必须禁用旧定义。本包以实验性名称公开发布，但不提供稳定性保证。
...
当模型应该通过工具运行一支团队时，在 `@deepseek-ai/dsh-experimental-agent-team` 之上挂载本包。挂载后，每个团队成员——Lead 与每个 teammate——都会获得相同的十四个工具，以及相同的协作策略。`spawn_teammate` 在初始任务前加上 teammate 的角色和名字。
...
- **创建或退出 teammate**——`spawn_teammate` 接收名字、描述、初始任务与可选的 `preset_id`；调用 `retire_teammate` 前必须处理未完成任务和待投递消息。只有 Lead 可以执行这两项操作。
- **发送消息**——`send_message` 在最近的步骤边界对运行中的成员进行 steering（中途引导）、启动或恢复非活动成员，也可关联一个真实 Task ID，而不改变投递路径。
- **查看与等待**——`list_agents` 返回各成员的 `target` 与可用状态；仅 Lead 可使用 `team_message_list` 分页读取私聊全文；`wait_agent` 等待下一次团队变化；`interrupt_agent` 停止 teammate 的当前轮次（仅限 Lead）。
- **管理任务板**——`team_task_create`、`team_task_list`、`team_task_get` 与 `team_task_update` 添加、浏览、读取与更新共享任务。Owner 使用 `team_task_submit_result` 单独提交结果；只有 Lead 能使用 `team_task_accept` 验收并放行下游依赖，或使用 `team_task_rework` 保留旧结果并新建重做 Task。即使 teammate 在同一 Session 继续对话，每次新派发或后续工作也会有独立 Task。

创建和列表结果使用 `target` 标识成员，不包含成员 Session ID。仅将 active 的 target 用于消息、中断调用或任务工具的 `owner` 参数；任务的 `ownerName` 使用相同值。`inactive` 表示没有轮次在执行，包括已加载和需要恢复的成员；它不表示任务完成或结果。`provisioning` 与 `failed` 描述创建状态；`retiring` 与 `retired` 保留在历史中，但不能接收 Team 工作。active 成员可相互发送消息并使用任务板；只有 Lead 可以创建、中断和让 teammate 退队。任务更新保留领域的 owner 与 revision 校验，因此过期的编辑会被拒绝，而不是覆盖更新的成果。
...
| 文件 | 职责 |
|---|---|
| [`src/index.ts`](src/index.ts) | 插件入口：配置、固定策略文本与十四个 scoped 工具注册 |
| — | 不发布运行时不变式伴生入口；Team 服务拥有持久化与授权关系。 |

### 策略与工具

member scope 上的一个 `team:policy` 段落说明共享的协作规则；固定文本与十四个工具注册都声明在 [`src/index.ts`](src/index.ts)。十四个工具 schema 注册在发布时被识别为 Team member 的 scope 中。与旧全局 continuable-subagent 控件同名的 scoped 注册只会为团队成员覆盖这些全局控件。
...
一段共享 system 策略会说明显式 delegation 要求、共享 cwd 行为、文件陈旧版本恢复、Bash／formatter／codegen 风险、task／write-scope 协调、Steer 投递、mailbox 不重试规则，以及 Lead 必须在回答前等待。Lead 与 teammate 的全部十四个 Team schema 相同；执行时检查仅限 Lead 的操作权限。`spawn_teammate` 在初始 user 消息前加上 `<system-reminder>\nYou are teammate "<name>".\n</system-reminder>`，接着是一个空行和任务。该前缀不含 Team id，禁用运行时上下文时也能生效。fork 继承历史，不额外添加 Lead 身份消息。
