---
description: "Fourteen tools that let the model create, retire, message, monitor, and coordinate teammates, for compositions mounting the experimental Team plugins."
...
This package lets the model create and retire named teammates, send messages, inspect Lead-visible private communication, wait for progress, interrupt stuck work, and coordinate through a shared task board. Every team member receives the same fourteen tools and guidance for coordinating in a shared workspace. Choose it when the model should operate a team only after you explicitly request one. It replaces legacy subagent controls with the same tool names, so compositions that need both must disable the legacy definitions. The package is published under its experimental name and provides no stability guarantee.
...
Add this package on top of `@deepseek-ai/dsh-experimental-agent-team` when the model should run a team through tools. Once mounted, every team member — the Lead and each teammate — gets the same fourteen tools plus the same coordination policy. `spawn_teammate` prefixes the initial task with the teammate’s role and name.
...
The fourteen tools group into four capabilities:

- **Create or retire a teammate** — `spawn_teammate` takes a name, a description, the initial task, and an optional `preset_id`; `retire_teammate` requires unfinished tasks and pending mail to be resolved first. Only the Lead can execute either operation.
- **Send messages** — `send_message` steers a running member at its nearest step boundary, starts or resumes an inactive member, and may link an existing Task ID without changing delivery.
- **See and wait** — `list_agents` returns each member’s `target` and availability; Lead-only `team_message_list` pages through complete private messages; `wait_agent` waits for the next team change; `interrupt_agent` stops a teammate's current turn (Lead only).
- **Manage the task board** — `team_task_create`, `team_task_list`, `team_task_get`, and `team_task_update` add, browse, read, and update shared tasks. The owner uses `team_task_submit_result` to submit a separate result; only the Lead uses `team_task_accept` to release downstream prerequisites or `team_task_rework` to create a new Task while preserving the old result. Every new assignment or follow-up gets its own Task, even when the teammate continues in the same Session.

Creation and listing results identify members by `target`, with no member Session ID. Use an active target in message and interrupt calls or the task tools’ `owner` parameter; task `ownerName` uses the same value. `inactive` means no turn is executing, whether the member is loaded or must be resumed; it does not describe task completion or outcome. `provisioning` and `failed` describe creation; `retiring` and `retired` remain visible for history but cannot receive Team work. Active members can message one another and use the task board; only the Lead creates, interrupts, and retires teammates. Task updates keep the domain's owner and revision checks, so an outdated edit is rejected instead of overwriting newer work.
...
| File | Role |
|---|---|
| [`src/index.ts`](src/index.ts) | Plugin entry: config, the fixed policy text, and the fourteen scoped tool registrations |
| — | No runtime invariant companion is published; the Team service owns durable and authorization relations. |

### Policy and tools

One `team:policy` section on the member scope states the shared coordination rules; the fixed text and the fourteen tool registrations are declared in [`src/index.ts`](src/index.ts). The fourteen tool schemas are registered in scopes recognized as Team members at publication. Scoped registrations with the same names as the legacy global continuable-subagent controls shadow those globals for team members only.
