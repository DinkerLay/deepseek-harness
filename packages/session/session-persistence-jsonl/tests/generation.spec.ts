...
function adapter(overrides: Partial<TestGenerationFormatAdapter> = {}): TestGenerationFormatAdapter {
  ...
  return {
    ...
    createRestore(headerValue) {
      ...
      const artifact = () => ({ header, inheritedEventCount: 0, events })
      return {
        header,
        decodeRow(row) { events.push(row as SessionFormatArtifact['events'][number]) },
        finish: artifact,
        finishWithCoordinates: () => ({
          artifact: artifact(),
          coordinates: {
            sourceVersion: Number(headerValue['version']),
            targetVersion: currentVersion,
            targetSeqBySourceSeq: events.map(event => event.seq),
    ...
    const wrong = options(wrongRoot, 'none', {
      ...
      createRestore: (headerValue) => {
        ...
        return {
          ...
          finishWithCoordinates: () => ({
            artifact: artifact(),
            coordinates: { sourceVersion: 0, targetVersion: 0, targetSeqBySourceSeq: [], targetEventCount: 0 },
          }),
        }
      },
    })
    ...
    await expect(ensureJsonlGenerationCurrent(options(wrongRoot, 'none', adapter({
      createRestore: (value) => {
        ...
        return {
          ...
          finishWithCoordinates: () => {
            const result = restore.finishWithCoordinates()
            return { ...result, artifact: { ...result.artifact, header: { ...result.artifact.header, version: 4 } } }
          },
        }
      },
