...
Sessions can persist as append-only JSONL with checksummed Zstandard frames or raw lines. Persistence handles present one current logical event stream while format decoding, migration coordinates, compression, and torn-tail recovery stay inside the backend. Permanent deletion removes owned generations under a write lease. The public legacy-SQLite exporter reads a released database in an isolated process, validates portable uncompressed JSONL, preserves original records, and publishes a separate Session root.
...
<root>/
  --<normalized-cwd>--/          # readable project directory (or _no-cwd/)
    <encoded-id>/                # session-owned directory
      ...
      session.v3.jsonl           # released v3, raw root
```
...
`open(id, 'read'|'write')` selects the highest canonical generation. Current input follows the ordinary fast path. For historical input, a read open decodes and migrates the source once, validates the current logical result, and returns it without publishing a successor. A write open reuses that revision-keyed preparation when available, or performs the same preparation, then encodes a same-directory temporary file in bounded chunks, verifies it in a Worker Thread, rechecks the source revision, and publishes the current successor without overwrite before returning. The source remains byte-identical. Source drift after preparation rejects that write open without replacing the logical history already returned to readers; a later write open prepares the new revision. `migrationCoordinates(id)` can repeat this read-only preparation after publication because immutable predecessors remain beside the current generation; it verifies only the migrated prefix, so later native appends do not prevent sidecar recovery. Its source revision is a `content-v1` SHA-256 identity over the exact predecessor bytes, format generation, and Session id, so a byte-identical copied home retains the same marker. The target revision and the `stat(id)`/`list()` revisions remain physical device, inode, size, and timestamp tokens. The backend marks decoded event graphs `shared-frozen` when it freezes them before memoization; handle reads and slices preserve that state, including empty slices. Only an unmaterialized pending log reports `detached`. `stat(id)` and `list()` select and translate only the highest generation header without reading event rows or starting migration; snapshots carry `sizeBytes`. With `compression: 'none'`, the log is newline-delimited text an external reader can consume directly; the compressed default must be read through the backend.

### Exporting recorded rc2 SQLite Sessions

The public `./legacy-sqlite` subpath exports `exportLegacySqlite({ runtimeRoot, database, destination, signal? })`. `runtimeRoot` must identify the recorded `0.1.1-rc.2` fork installation. The helper rejects an existing destination, backs up the source database read-only, runs that version-checked reader in a separate process, and writes a new uncompressed JSONL root. It never changes or deletes the original database; configure the returned `sessionRoot` with `compression: 'none'`.

The rc2 SQLite writer can contain one exact chunk attempt immediately before its final Assistant message without the later `sourceEventSeqs` envelope field. The export adds that field only when every chunk is contiguous and belongs to the same Turn and step. It verifies the model source and assembled message, usage, and replay state, then refuses gaps, cross-step chunks, or conflicting provenance. The ordinary JSONL migration remains strict and does not admit a generic missing-provenance file.
...
| [`src/legacy-sqlite.ts`](src/legacy-sqlite.ts) | Public read-only database backup, isolated reader lifecycle, and destination publication |
| [`src/legacy-sqlite-worker.ts`](src/legacy-sqlite-worker.ts) | Version-checked rc2 reader and portable JSONL writer |
| [`src/legacy-sqlite-provenance.ts`](src/legacy-sqlite-provenance.ts) | Strict reconstruction of provable rc2 Assistant chunk provenance |
...
- **SQLite export requires the recorded runtime** — `exportLegacySqlite` refuses another DSH package version; deployments must retain or supply the version-checked rc2 installation for this offline conversion.
