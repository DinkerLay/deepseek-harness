...
import { open, mkdir, readdir, realpath, link, rm, stat, truncate, unlink } from 'node:fs/promises'
...
import {
  ...
  type SessionMigrationCoordinates,
...
class JsonlSessionPersistence extends SessionPersistence {
  ...
  override readonly supportsMigrationCoordinates = true
   ...
   */
  override async migrationCoordinates(
    id: SessionId,
    ...
    signal?.throwIfAborted()
    await this.ensureRootEncoding()
    const selected = await this.findHistoricalLog(id, signal)
    if (selected === undefined) return undefined
    ...
    if (await this.exists(selected.currentPath)) {
      const current = await this.readStoredLog(selected.currentPath, id, signal)
      const migratedPrefixMatches = prepared.artifact.events.every((event, index) =>
        isDeepStrictEqual(current.events[index], event))
      if (!isDeepStrictEqual(current.meta, prepared.artifact.header)
        || current.inheritedEventCount !== prepared.artifact.inheritedEventCount
        || current.events.length < prepared.artifact.events.length
        || !migratedPrefixMatches) {
        throw new Error(`session "${id}": current generation does not retain its migrated predecessor prefix`)
      }
      targetRevision = fileRevision(await stat(selected.currentPath, { bigint: true }))
    }
    const targetSeqBySourceSeq = prepared.coordinates.targetSeqBySourceSeq.map(value =>
      value === null ? null : SessionSeq(value))
    return Object.freeze({
      sessionId: id,
      source: Object.freeze({
        version: prepared.coordinates.sourceVersion,
        revision: SessionPersistenceRevision(prepared.sourceContentIdentity),
      }),
      target: Object.freeze({
        version: prepared.coordinates.targetVersion,
        ...(targetRevision === undefined ? {} : { revision: targetRevision }),
      }),
      targetSeqBySourceSeq: Object.freeze(targetSeqBySourceSeq),
      targetEventCount: SessionLogOffset(prepared.coordinates.targetEventCount),
    })
  }

  /**
   * Permanently remove every canonical generation for one Session. Deletion
   * claims native write ownership first, holds the cross-process lease while
   * removing oldest-to-newest, and publishes cleanup only after the newest
   * generation is gone.
   * @param id - stored Session identity to remove.
   * @param options - optional cancellation observed before the first unlink.
   * @returns the removed logical header, or `undefined` when absent.
   */
  override async delete(
    ...
    try {
      ...
      if (selected === undefined) return undefined
      lease = await this.acquireLease(id, undefined, dirname(selected.currentPath))
      const stored = await this.requireStoredLog(id, signal)
      signal?.throwIfAborted()
      const directory = dirname(selected.currentPath)
      const generations = (await readdir(directory, { withFileTypes: true }))
        .flatMap((entry) => {
          if (!entry.isFile() && !entry.isSymbolicLink()) return []
          const version = parseGenerationLogFilename(entry.name, this.compression)
          return version === undefined ? [] : [{ path: join(directory, entry.name), version }]
        })
      ...
      if (preparation !== undefined) {
        ...
        preparation.controller.abort(new Error(`session "${id}" was permanently deleted`))
      }
      const header = structuredClone(stored.meta)
      try {
        await this.ctx.parallel('session-persistence/deleted', header, stored.inheritedEventCount)
      } catch (error: unknown) {
  ...
  /** Select the newest retained predecessor in one Session directory. */
  private async resolveHistoricalGenerationInDirectory(
    dir: string,
    signal?: AbortSignal,
  ): Promise<ResolvedJsonlGeneration | undefined> {
    let entries: Dirent[]
    ...
    signal?.throwIfAborted()
    const historical = entries.flatMap((entry) => {
      if (!entry.isFile()) return []
      const version = parseGenerationLogFilename(entry.name, this.compression)
      return version === undefined || version >= sessionFormatCatalog.currentVersion
        ? []
        : [{ name: entry.name, version }]
    }).sort((left, right) => right.version - left.version)[0]
    if (historical === undefined) return undefined
    return {
      sourcePath: join(dir, historical.name),
  ...
  /** Find one unique retained historical generation for a Session id. */
  private async findHistoricalLog(
    id: SessionId,
    signal?: AbortSignal,
  ): Promise<ResolvedJsonlGeneration | undefined> {
    const matches: ResolvedJsonlGeneration[] = []
    for (const project of await this.listProjectDirs(signal)) {
      ...
      await this.rejectLegacyFlatArtifact(project, id, signal)
      const dir = join(project, encodeSegment(id))
      const selected = await this.resolveHistoricalGenerationInDirectory(dir, signal)
      if (selected !== undefined) matches.push(selected)
    }
    if (matches.length > 1) {
      throw new Error(`duplicate JSONL session id "${id}" appears in multiple project directories`)
