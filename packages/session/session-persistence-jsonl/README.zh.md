...
Session 可以持久化为带 checksum 的 Zstandard frame 或原始行组成的只追加 JSONL。Persistence handle 提供一条当前逻辑事件流，格式解码、迁移坐标、压缩与断尾恢复由后端内部处理。永久删除在写 lease 下移除自有 generation。公共 legacy-SQLite exporter 在隔离进程中读取已发布数据库，校验可移植的未压缩 JSONL，保留原始记录，并发布独立 Session 根目录。
...
`open(id, 'read'|'write')` 选择最高规范 generation。当前格式输入走普通快速路径。对于历史输入，只读 open 会单遍解码并迁移源、校验当前逻辑结果，然后在不发布后继的情况下返回。写 open 会在可用时复用按 revision 为键的 preparation，否则执行同一套 preparation，再按有界分片编码同目录临时文件、在 Worker Thread 中校验、复查源修订，并在返回前以不覆盖方式发布当前后继。源保持逐字节不变。如果源在 preparation 后发生变化，该次写 open 会失败，已经返回给读方的逻辑历史不会被替换；后续写 open 会针对新的 revision 重新执行 preparation。由于不可变前任仍与当前 generation 并存，`migrationCoordinates(id)` 可以在发布后重复这项只读 preparation；它只校验迁移后的前缀，因此后续原生追加不会阻止 sidecar 恢复。其 source revision 是覆盖精确前代字节、格式 generation 与 Session id 的 `content-v1` SHA-256 身份，因此逐字节相同的 home 副本会保留同一标记。Target revision 与 `stat(id)`/`list()` revision 仍是 device、inode、size 和 timestamp 组成的物理 token。后端在 memo 化前冻结已解码的 event graph，并在此时将其标记为 `shared-frozen`；句柄读取和 slice 即使为空也保留该状态。只有尚未实体化的 pending 空日志报告 `detached`。`stat(id)` 与 `list()` 只选择并转换最高 generation 的 header，不读取事件行，也不启动迁移；snapshot 携带 `sizeBytes`。选择 `compression: 'none'` 后，日志是外部读取方可直接消费的换行分隔文本；压缩默认值必须经后端读取。

### 导出记录的 rc2 SQLite Session

公共 `./legacy-sqlite` 子路径导出 `exportLegacySqlite({ runtimeRoot, database, destination, signal? })`。`runtimeRoot` 必须指向记录的 `0.1.1-rc.2` fork 安装。该 helper 会拒绝已有 destination，以只读方式备份源数据库，在独立进程中运行经过版本检查的 reader，并写入新的未压缩 JSONL 根目录。它绝不更改或删除原始数据库；应使用 `compression: 'none'` 配置返回的 `sessionRoot`。

rc2 SQLite writer 可以在最终 Assistant 消息前直接保存一个精确 chunk attempt，但没有后续引入的 `sourceEventSeqs` envelope 字段。只有每个 chunk 连续且属于同一 Turn 与 step 时，export 才会补充该字段。它校验模型来源以及组装后的 message、usage 与 replay state，并拒绝 gap、跨 step chunk 或冲突 provenance。普通 JSONL migration 继续保持严格，不会接受一般性的 provenance 缺失文件。
...
| [`src/legacy-sqlite.ts`](src/legacy-sqlite.ts) | 公共只读数据库备份、隔离 reader 生命周期与 destination 发布 |
| [`src/legacy-sqlite-worker.ts`](src/legacy-sqlite-worker.ts) | 经过版本检查的 rc2 reader 与可移植 JSONL writer |
| [`src/legacy-sqlite-provenance.ts`](src/legacy-sqlite-provenance.ts) | 严格重建可证明的 rc2 Assistant chunk provenance |
...
- **SQLite export 需要记录的 Runtime**——`exportLegacySqlite` 会拒绝其他 DSH package 版本；部署方必须为这项离线转换保留或提供经过版本检查的 rc2 安装。
