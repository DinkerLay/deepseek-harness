...
永久 `delete(id)` 会等待同 id retirement 与 preparation work，然后持有 `BEGIN IMMEDIATE`、重新校验 schema ownership，并删除选中的 Session row。Foreign-key cascade 会在同一个 transaction 中移除其 event row；row 已不存在时是幂等 no-op。
...
- **没有后台历史压缩**——支持显式 Session deletion，普通 append 仍然只做 insert。
