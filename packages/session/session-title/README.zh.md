...
Fork 继承临时标题，并根据种子之后的输入生成自己的标题。继承的用户锁定仅对来源 Session 生效。显式 refresh 通过 `session/title-policy` 解除用户锁定；`session/title-generation` 记录生成中、成功和失败，不派发任务输入。

`registerAutomaticMode` 为部署拥有的 Session 角色选择命名节奏，同时保留用户显式锁定和精确日志路由。`titleGeneration` 投影公开生成中、成功和失败状态。命名中断可以恢复，不会派发任务输入。
