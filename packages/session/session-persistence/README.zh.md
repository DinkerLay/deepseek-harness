...
应用可以通过与后端无关的 handle API 持久化并恢复只追加 Session 日志。一个 writer 拥有 Session，reader 获得经过校验的连续前缀；flush 是耐久性屏障。Provider 可以提供永久删除与精确迁移坐标观测。删除会向派生消费方报告被移除的生命周期；迁移坐标把不可变源 revision 映射到当前日志，而不发布或改写任一历史。
