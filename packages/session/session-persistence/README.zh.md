...
| `supportsDeletion: boolean` | 明确说明该后端是否实现串行化永久删除；调用方绝不根据方法是否存在来推断支持。 |
...
| `delete(id): Promise<SessionHeader \| undefined>` | 与同 id 写入、preparation 和 retirement 串行执行；拒绝 live 或被独占 preparation 的 identity；提交后移除持久 artifact 或 lazy creation intent，并发出 `session-persistence/deleted`。移除已知 identity 时返回其 header；重试删除已不存在的 identity 时返回 `undefined`。 |
...
| `listDeletionHeaders(): Promise<SessionHeader[]>` | 为递归删除发现列出 coordinator 已知的 materialized、lazy 与已加载 prepared header。直接实现的后端默认回退到 `list()`。 |
...
| `deleteStored?(id)` | 永久移除一个 materialized Session 并返回其 header。JSONL unlink 精确日志；SQLite 在一个事务中删除 Session 行与级联拥有的 event。 |
...
- **保留策略仍在本服务之外**：服务可以删除经过明确授权的 Session，但不会选择保留周期、归档状态或产品 ownership。
