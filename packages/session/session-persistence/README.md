...
| `supportsDeletion: boolean` | State explicitly whether this backend implements serialized permanent deletion; callers never infer support from method presence. |
...
| `delete(id): Promise<SessionHeader \| undefined>` | Serialize with same-id writes, preparations, and retirement; reject a live or exclusively prepared identity; remove its durable artifact or lazy creation intent and emit `session-persistence/deleted` after commit. A removed known identity returns its header; a retry over an absent identity returns `undefined`. |
...
| `listDeletionHeaders(): Promise<SessionHeader[]>` | List materialized, lazy, and loaded prepared headers known to a coordinator for recursive deletion discovery. A direct backend defaults to `list()`. |
...
| `deleteStored?(id)` | Permanently remove one materialized Session and return its header. JSONL unlinks the exact log; SQLite deletes the Session row and cascade-owned events in one transaction. |
...
- **Retention policy remains external** — the service can delete an explicitly authorized Session, but it does not choose retention periods, archive state, or product ownership.
