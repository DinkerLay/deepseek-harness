...
export interface PersistenceBackend<TornMarker = unknown> {
   ...
   */
  deleteStored?(id: SessionId): Promise<SessionHeader | undefined>
...
export class PersistenceCoordinator<TornMarker = unknown> {
  ...
  private async deleteCore(id: SessionId): Promise<SessionHeader | undefined> {
    ...
    const preparedHeader = this.preparations.invalidateForDeletion(id)
    if (this.backend.deleteStored === undefined) {
      throw new Error(`${this.backend.name} does not support permanent Session deletion`)
    }
    const storedHeader = await this.backend.deleteStored(id)
    const deleted = storedHeader ?? state?.meta ?? preparedHeader
  ...
  async listDeletionHeaders(): Promise<SessionHeader[]> {
    ...
    for (const state of this.states.values()) headers.set(state.meta.id, state.meta)
  ...
  private async emitDeleted(header: SessionHeader): Promise<void> {
    try {
      await this.ctx.parallel('session-persistence/deleted', header)
    } catch (error: unknown) {
