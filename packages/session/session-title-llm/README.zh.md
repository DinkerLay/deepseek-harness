...
调用方可以在统一模型策略下，根据选定的人类消息生成简洁的 Session 标题。调用方选择参与消息，并提供模型路由或使用 Session 路由。必需的字节、输出 token 与时长上限约束请求；流式期间取消始终有效。无效或非文本结果会被拒绝；Provider 有意缩短源文本时，接受的标题会记录该事实。
