...
Provider message selection is independent of automatic cadence. `registerSessionTitleLlmProvider()` always honors its selector. `sessionTitleInputBytes()` measures the complete framed input so deployment providers can choose their own retention and excerpt policy. The executor records shortened input through `inputTruncated` and performs no clipping itself.
