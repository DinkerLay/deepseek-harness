...
完成后的格式 restore 通过 `finishWithCoordinates()` 公开同一组已审计位置：每个源事件对应一个目标事件序号；如果较早迁移边消费了某个源事件（例如内嵌的 Assistant 分片），则对应值为 `null`。该映射绝不解释无关数值字段。
...
迁移分类[已发布 V2 事件清单](../session-format-v1-to-v2/src/dispositions.ts)，包括仅日志的 `assistant/attempt`、`feedback/message-put`、`feedback/message-delete`，以及已记录的 fork 扩展 `session/execution-directory`、`session/title-policy` 和 `session/title-generation`。标题与标题请求记录只接纳经过校验的 `inputTruncated: true` 扩展。[载荷校验器](src/payload.ts)应用精确的已接纳信封和载荷成员，以及已发布嵌套校验。未知事件（即使可忽略）以及被检查记录中未经审计的成员均被拒绝。消息来源分类覆盖下表的五个 Message 位置：未知来源种类会被拒绝；用户来源和 agent（智能体）中继来源只保留经过校验的 `delegation` 记录，其中 `parentSessionId` 必须非空，`parentTurn` 可选且必须为非负整数。
