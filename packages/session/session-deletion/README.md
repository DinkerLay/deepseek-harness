---
description: "Host-only permanent Session deletion and reservations."
...
No invariant companion is published: deletion consumes its authoritative relations in one operation and is covered by lifecycle regressions.

## Table of Contents

- [Operation](#package-section-1)
- [Composition](#package-section-2)
- [Model Experience](#package-section-3)
- [Known Limitations and Deferred Work](#package-section-4)
- [Dev Note](#package-section-5)
...
Known persistence identities delete bottom-up through `ctx.sessionPersistence.delete()`. JSONL takes native write ownership and the cross-process lease, then removes every canonical generation while retaining its stable lock file; a closed, unmaterialized identity has no artifact to delete. Every identity removed by persistence publishes `session-persistence/deleted`. A zero-event live Session may already have retired its lazy intent during Agent disposal, so it remains in `sessionIds` but not `deletedSessionIds`. A retry skips absent records and converges on root deletion. The package never archives, changes Workspace placement, removes product allocations, or deletes Project directories.
