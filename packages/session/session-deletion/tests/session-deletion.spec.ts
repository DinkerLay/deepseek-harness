...
async function mounted(kind: 'jsonl' | 'sqlite'): Promise<{ readonly ctx: Context; readonly cwd: string }> {
...
for (const kind of ['jsonl', 'sqlite'] as const) {
  describe(`SessionDeletion (${kind})`, () => {
    it('deletes a complete persisted subtree bottom-up', async () => {
      const { ctx, cwd } = await mounted(kind)
      const root = header(`${kind}-root`, cwd)
      const child = header(`${kind}-child`, cwd, root.id)
      const grandchild = header(`${kind}-grandchild`, cwd, child.id)
    ...
    it('deletes a lazy Session identity and notifies derived consumers', async () => {
      const { ctx, cwd } = await mounted(kind)
      const root = header(`${kind}-lazy-root`, cwd)
    ...
    it('rejects an unretained live Session before deleting durable state', async () => {
      const { ctx, cwd } = await mounted(kind)
      const meta = header(`${kind}-unretained`, cwd)
    ...
    it('claims a retained idle lifecycle and then deletes its durable record', async () => {
      const { ctx, cwd } = await mounted(kind)
      const meta = header(`${kind}-tracked`, cwd)
    ...
    it('releases every earlier live claim when a later subtree member is busy', async () => {
      const { ctx, cwd } = await mounted(kind)
      const rootMeta = header(`${kind}-atomic-root`, cwd)
      const childMeta = header(`${kind}-atomic-child`, cwd, rootMeta.id)
...
describe('SessionDeletion recovery and lineage validation', () => {
  it('rejects an unsupported persistence provider before touching a live Session', async () => {
    const { ctx, cwd } = await mounted('jsonl')
