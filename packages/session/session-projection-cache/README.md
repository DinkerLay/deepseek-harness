...
Restarted applications can resume Session projections from durable checkpoints instead of folding every log from the beginning. The Session log remains authoritative: stale or incompatible checkpoints are ignored or backed up and can never advance beyond committed events. Committed deletion removes only the matching lifecycle checkpoint and fences delayed live or cold writes from recreating it. Skip this cache when projections are live-only or retention cost exceeds saved work.
