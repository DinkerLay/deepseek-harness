...
重启后的应用可以从持久 checkpoint 恢复 Session projection，无需从头折叠每条日志。Session 日志仍是权威：陈旧或不兼容的 checkpoint 会被忽略或备份，并且绝不能超越已提交事件。已提交删除只移除匹配生命周期的 checkpoint，并阻止延迟的 live 或 cold 写入重新创建它。Projection 仅用于 live 状态或保留成本超过收益时应跳过该缓存。
