...
Agents can run sequential PowerShell commands in an isolated shell whose directory, environment, functions, and background jobs persist across calls. Choose it for multi-step PowerShell state; use one-shot PowerShell for clean commands or a terminal for interactive stdin. Timeout or `exit` resets the shell. Its initial directory follows the recorded Session execution directory, while agentless calls keep backend defaults.
