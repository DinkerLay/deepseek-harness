...
Agent 可以使用原生 Windows 路径与环境语法运行一次性 PowerShell 命令，也可以启动后台任务供后续检查和终止。每次调用使用全新进程。Sandbox 会执行 Windows language-mode 与 named-pipe 规则，部署 constraint 限制获批访问上限。执行跟随记录的 Session 目录，并可在创建进程前立即收集可信环境值，而不在模型输入中暴露它们。
