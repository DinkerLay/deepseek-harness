...
Agents can run one-shot PowerShell commands with native Windows paths and environment syntax, or start background jobs for later inspection and termination. Each call uses a fresh process. Sandboxing enforces Windows language-mode and named-pipe rules, while deployment constraints cap approved access. Execution follows the recorded Session directory and can collect trusted environment values immediately before process creation without exposing them in model input.
