...
Agent 可以运行一次性 Bash 命令并接收 stdout、stderr 与退出标记，也可以启动后台任务供后续检查和终止。每次调用使用全新 shell。Sandbox 拒绝支持一次经批准的更宽权限重试，但部署 constraint 仍限制授权上限。执行跟随记录的 Session 目录，并可在创建进程前立即收集可信环境值，而不在模型输入中暴露它们。
