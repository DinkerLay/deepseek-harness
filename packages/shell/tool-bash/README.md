...
| `workdir` | string | Working directory for this call. Defaults to the filesystem identity of the calling agent's session cwd (`resolveSessionCwd(session)`) so each session runs in its own workspace; a relative `workdir` is resolved against that same identity. |
