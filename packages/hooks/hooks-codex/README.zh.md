...
现有 Codex 命令 hook 可以对 Session 启动、prompt 提交、工具执行与 stop 设置门禁，无需改写。Hook 可以用模型可见原因阻止 prompt 或工具、加入上下文，或请求另一个 Agent step。命令从记录的 Session 执行目录运行。超出这五个受支持 Codex hook 点的行为应使用原生插件。
