...
现有 Claude Code 命令 hook 可以在匹配的 Session、prompt、工具、stop 与 subagent 时机运行，无需改写。Hook 可以用模型可见原因阻止 prompt 或工具、加入上下文，或请求另一个模型 Turn。命令从记录的 Session 执行目录运行。超出受支持 Claude Code hook 合同的行为应使用原生插件。
