...
Existing Claude Code command hooks can run at matching Session, prompt, tool, stop, and subagent moments without a rewrite. Hooks may block prompts or tools with model-visible reasons, add context, or request another model turn. Commands run from the recorded Session execution directory. Choose a native plugin for behavior outside the supported Claude Code hook contract.
