...
export function apply(ctx: Context, config: Config = {}): void {
  ...
  const skillTool = defineTool({
    ...
    async execute(args, exec) {
      ...
      const lookup = { cwd: resolveSessionCwd(exec.agent?.session), signal: exec.signal, scope: exec.agent }
  ...
  ctx.on('agent/pre-step', async (
    ...
    const lookup = { cwd: resolveSessionCwd(agent.session), signal, scope: agent }
  ...
  ctx.on('agent/pre-step', async (
    ...
    const snapshot = toolVisible
      ? await ctx.skills.snapshot({ cwd: resolveSessionCwd(agent.session), signal, scope: agent })
