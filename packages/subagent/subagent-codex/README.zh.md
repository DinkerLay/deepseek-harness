...
委托工作可以在 parent 记录的执行目录中作为全新、无人值守的 Codex thread 运行。每次运行接受一个自包含任务，只返回最终答案或安全诊断。原生 Codex 配置与鉴权仍是权威，权限模式选择非交互审批与 sandbox 行为。Bundle 提供兼容的固定 payload，但在配置 delegation 工具前不公开模型 capability。
