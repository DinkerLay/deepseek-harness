...
import type { SubagentParentDelivery, SubagentParentDeliveryPolicy } from './continuation-messages.ts'
...
export class SubagentRuntime extends TypertRemoteService {
  ...
  private quietParentDelivery(request: SubagentParentDelivery): boolean {
    ...
    for (const registration of [...this.deliveryPolicies]) {
      ...
      try {
        if (registration.policy(request) === 'quiet') quiet = true
      } catch (error: unknown) {
        quiet = true
