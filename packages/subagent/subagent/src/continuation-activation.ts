...
import type { SubagentParentDelivery } from './continuation-messages.ts'
...
export class ContinuableActivationRegistry {
  ...
  constructor(
    ...
    private readonly quietParentDelivery: (delivery: SubagentParentDelivery) => boolean,
  ) {
  ...
  private notifySettlement(activation: Activation, terminal: ActivationTerminal): void {
    ...
    try {
      ...
      if (this.closingTeardownFor(parent) !== undefined
        || this.quietParentDelivery({ parent, childSessionId: activation.childId, kind: 'settlement' })) {
