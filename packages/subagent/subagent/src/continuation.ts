...
interface ContinuationHost {
  ...
  quietParentDelivery(delivery: import('./continuation-messages.ts').SubagentParentDelivery): boolean
...
export class SubagentContinuationManager {
  ...
  constructor(
    ...
    this.activations = new ContinuableActivationRegistry(
      ...
      delivery => host.quietParentDelivery(delivery),
    )
  }
  ...
  private sendToParent(
    ...
    if (this.host.quietParentDelivery({ parent, childSessionId: sender.id, kind: 'message' })) {
  ...
  private submitAdmitted(
    ...
    const delegation = currentParentDelegation(parent)
    const message = options.source === undefined
      ? createAgentMessage(parent, content, delegation)
