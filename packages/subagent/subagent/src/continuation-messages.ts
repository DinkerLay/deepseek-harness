...
export interface AgentMessageSource {
  ...
  /** Parent location that accepted this delegated message. */
  readonly delegation?: SubagentMessageDelegation
}

/** Durable parent location captured when a parent-to-child message is accepted. */
export interface SubagentMessageDelegation {
  readonly parentSessionId: SessionId
  /** Open parent Turn when delivery was accepted; absent between Turns. */
...
/** Quiet retains the accepted input without waking; undefined preserves ordinary scheduling. */
 ...
 * @param delegation - optional durable parent Session and Turn attribution.
 * @returns the durable user-message representation delivered to the recipient.
 */
export function createAgentMessage(
  sender: Agent,
  content: ContentBlock[],
  delegation?: SubagentMessageDelegation,
...
/**
 * Capture the exact parent Session and its currently open Turn for one accepted delivery.
 * @param parent - exact live parent authorizing the delivery.
