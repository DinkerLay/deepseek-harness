...
应用可以把工作委托给具名的一次性或可继续 child Agent，检查其活动与谱系，发送后续消息，并中断受支持的 child。多个进程内与子进程后端可以共存。已接受的 child 消息会记录 parent Session 与未闭合 Turn。Parent 投递策略可以静默保留消息与结算通知，而不改变内容或唤醒 parent；移除策略会恢复普通调度。
