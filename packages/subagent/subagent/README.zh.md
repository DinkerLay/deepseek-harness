...
独立的 `@deepseek-ai/dsh-subagent/host-owned` 服务允许受信任 Host 控制器使用显式 Preset 创建委派 Agent，并在发起 Agent 销毁后继续持有它们。必填配置 `maxDepth` 限制委派深度。该服务需要 Agent 工厂、Session 持久化/查询/投影与 Preset registry，不需要普通 `subagents` 服务。创建时捕获来源 Agent 的委派权限策略，记录必需的所有权描述，沿用委派提示与限制。普通父级持有的 continuation 不能接管这些会话。

控制器提供持久 owner id 和稳定投递 request id。`send` 使用已有 Agent-message 来源记录实际发送者，在 Session flush 后返回；重复 id 返回原 message id、拒绝内容变更，并可唤醒仍待处理的输入而不重复插入。`interrupt` 等待执行静止；`retire` 记录永久关闭投递并释放 handle，不删除历史。冷恢复在发布前校验所有权、退役状态和 Preset 声明摘要；声明不可用或变化时拒绝恢复，不保留旧插件二进制或环境值。此入口不暴露模型工具或浏览器 RPC，仅供单 Host 使用；消费它的控制器负责调用者授权、业务成员资格、任务结果与重试决定。
