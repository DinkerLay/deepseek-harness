...
### Host-owned delegated execution

The separate `@deepseek-ai/dsh-subagent/host-owned` service lets a trusted Host controller create delegated Agents with an explicit Preset and retain them after the originating Agent is disposed. Its required `maxDepth` config bounds delegation. It needs the Agent factory, Session persistence/query/projections, and Preset registry; it does not require the ordinary `subagents` service. Creation captures the source Agent's delegated permission policy, records a required ownership descriptor, and keeps the same delegation prompt and restrictions. Ordinary parent-owned continuation cannot adopt these sessions.

The controller supplies its durable owner id and a stable delivery request id. `send` records the actual sender using the existing Agent-message source and returns after the Session flush; duplicate ids return the original message id, reject changed content, and can wake still-pending input without inserting it again. `interrupt` waits for quiescence; `retire` records permanent delivery closure and releases the handle without deleting history. Cold resume checks ownership, retirement and the Preset declaration digest before publishing. Unavailable or changed declarations reject recovery; no old plugin binaries or environment values are preserved. This entry exposes no model tools or browser RPC and is single-Host: the consuming controller must authorize callers and own business membership, task outcome and retry decisions.
