...
Child-directed messages retain `source.delegation.parentSessionId` and the open `parentTurn` at each acceptance, including repeated sends and cold continuations. `registerParentDeliveryPolicy()` can keep child messages and settlement notices quiet without changing their content; the callback receives `kind: message` or `settlement`. Quiet injection does not wake the parent, callback failure retains quietly, and disposal restores official scheduling. It is not content suppression.
