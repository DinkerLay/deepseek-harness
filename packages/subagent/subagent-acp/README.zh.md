...
委托任务可以在全新的 ACP 兼容子进程中运行，并拥有自己的 Runtime、Session、模型与工具。Child 只共享选定的工作目录，并返回最终答案或安全错误；中间流量留在 parent 对话之外。配置策略会无交互地回答权限 prompt。Child 必须共享 parent capability 时应选择进程内后端。工作跟随记录的 Session 执行目录。
