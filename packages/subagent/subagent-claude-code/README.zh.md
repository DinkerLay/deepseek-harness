...
委托工作可以在 parent 记录的执行目录中作为全新、无人值守的 Claude Code Session 运行。每次运行接受一个自包含任务，只返回最终答案或安全诊断；推理、工具流量、stderr、用量与 diff 留在 parent Session 之外。原生 Claude 设置与鉴权仍是权威，profile 配置选择模型、环境与权限模式。固定 Runtime 绝不回退到 Host 可执行文件。
