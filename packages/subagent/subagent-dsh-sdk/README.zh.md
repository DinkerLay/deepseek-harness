...
每个委托任务可以在全新的 DeepSeek Harness 子进程中运行，并拥有自己的 profile、Session、模型路由与工具。Child 接收任务与记录的执行目录，隔离中间消息和工具流量，并返回最终 assistant 文本或安全错误。需要完整 Harness Runtime 时选择此后端；共享状态比子进程隔离与启动成本更重要时选择进程内执行。
