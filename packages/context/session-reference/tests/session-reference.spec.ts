...
describe('session reference discovery and preparation', () => {
  ...
  it('labels a cold session from its checkpoint and reads no log', async () => {
    ...
    vi.spyOn(ctx.sessionQuery, 'listSessions').mockResolvedValue([
      { header: cold, executionDirectory: cold.cwd, live: false, persisted: true },
    ] as never)
    const readTitles = vi.spyOn(ctx.sessionQuery, 'readTitleSnapshots')
  ...
  it('labels a session no projection answers for by its id, still without a log read', async () => {
    ...
    vi.spyOn(ctx.sessionQuery, 'listSessions').mockResolvedValue([
      { header: seeded, executionDirectory: seeded.cwd, live: false, persisted: true },
    ] as never)
    const readTitles = vi.spyOn(ctx.sessionQuery, 'readTitleSnapshots')
  ...
  it('uses effective execution directories for discovery and referenced snapshot metadata', async () => {
    const ctx = await harness()
    const target = ctx.sessions.create(SessionId('target'), { meta: { cwd: '/shared-target' } })
    const source = ctx.sessions.create(SessionId('source'), { meta: { cwd: '/shared-source' } })
    target.append('session/execution-directory', { sessionId: target.id, cwd: '/branches/active' })
    source.append('session/execution-directory', { sessionId: source.id, cwd: '/branches/active' })
    appendText(source, 'branch fact')
    ...
    expect(promptData(contextText(prepared))).toMatchObject([{
      sessionId: source.id,
      cwd: '/branches/active',
    }])
  })
