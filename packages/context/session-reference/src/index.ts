...
export class SessionReferenceResolver extends TypertRemoteService {
  ...
  async listCandidates(
    ...
    const targetCwd = resolveSessionCwd(agent.session)
    ...
    const labelled = records.map(({ record, index }) => ({
      record,
      index,
      cwd: record.executionDirectory,
      label: this.projectedTitle(record) ?? record.header.id,
    }))
    return labelled.filter(({ record, cwd, label }) => {
      if (needle === '') return true
      return record.header.id.toLocaleLowerCase().includes(needle)
        || cwd?.toLocaleLowerCase().includes(needle) === true
        || label.toLocaleLowerCase().includes(needle)
    }).sort((a, b) => candidateRank(a.cwd, targetCwd) - candidateRank(b.cwd, targetCwd)
      || a.index - b.index)
      .slice(0, limit)
      .map(({ record, cwd, label }) => ({
        sessionId: record.header.id,
        label,
        ...(cwd === undefined ? {} : { cwd }),
        sameWorkspace: cwd !== undefined && cwd === targetCwd,
