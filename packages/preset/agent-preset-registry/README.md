...
The lease also exposes `compositionRows` from that same retained generation. A caller can inspect enabled plugin modules before creating an Agent without racing a separately queried current declaration against the revision it will mount.
