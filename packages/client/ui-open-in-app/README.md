---
description: "Web Session-header \"Open In...\" split button: launches the remembered application on the Session's effective execution directory and lists every application the host probed as installed."
...
This package provides the browser surface of the open-in-app feature: a Session-header split button whose main button opens the current Session's effective execution directory (the summary's `cwd`) in the remembered application, and whose chevron lists every catalog application the host probed as installed. Availability, icons, and launches come from the host routes of [`dsh-host-open-in-app`](../../host/open-in-app/README.md); mount the two packages together. A Session without an execution directory, or a host where nothing nameable is installed, renders no button at all.
...
Mount this plugin in the Web composition beside [`dsh-host-open-in-app`](../../host/open-in-app/README.md); the pair composes the whole feature in two cordis.yml rows and this row takes no config. The Session header grows an "Open In..." split button whenever the host probed at least one installed catalog application and the Session summary has a known effective execution directory.
