...
Browser clients can issue Remote calls, receive exact Fetch responses, observe connection generations, and request immediate reconnects. A generation becomes visible only after its source is ready and disappears on completion, failure, withdrawal, or stop before retry policy runs. Host handlers may require loopback authority for one channel while retaining browser authentication; handlers without that restriction keep authenticated trusted-host access.
...
Dedicated `connection.rpc.handle` channels reserve their names immediately and mount their HTTP routes when `webServer` is available. Consumers inject only `connection`; disposing a consumer removes its route and reservation. Duplicate channel names reject synchronously.
