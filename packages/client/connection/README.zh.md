...
浏览器客户端可以发出 Remote 调用、接收精确 Fetch 响应、观察连接 generation，并请求立即重连。Generation 只在其来源就绪后可见，并会在完成、失败、撤回或停止时先消失，再执行重试策略。Host handler 可以在保留浏览器鉴权的同时要求某一通道具有 loopback 权限；未设置该限制的 handler 保持经过认证的可信 Host 访问。
...
独立的 `connection.rpc.handle` 通道立即预留通道名，并在 `webServer` 可用时挂载 HTTP 路由。消费者只需注入 `connection`；消费者释放时会删除其路由和通道名预留。重复通道名会同步报错。
