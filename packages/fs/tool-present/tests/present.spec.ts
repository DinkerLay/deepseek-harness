...
describe('present', () => {
  ...
  it('resolves relative deliveries from the Session execution directory', async () => {
    const { owner, execute } = await setup()
    const worktree = await mkdtemp(join(tmpdir(), 'dsh-present-execution-'))
    cleanups.push(() => rm(worktree, { recursive: true, force: true }))
    await writeFile(join(worktree, 'result.txt'), 'branch result')
    owner.session.append('session/execution-directory', { sessionId: owner.id, cwd: worktree })
    expect((await execute([{ path: 'result.txt' }])).isError).toBe(false)
    expect(owner.session.snapshotEvents().find(event => event.type === 'deliverables/presented')?.data.files)
      .toEqual([{ path: 'result.txt' }])
  })
