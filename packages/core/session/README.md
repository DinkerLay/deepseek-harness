...
Applications can keep a reconstructable append-only record of model-visible facts, derive current history, replay, fork, flush, and inspect Sessions. Compaction hides superseded context without deleting it; a persistence backend adds restart durability. Recorded execution directories separate physical work from immutable creation identity. Deletion reservations fence stale publication, and publication checks carry that fence across asynchronous reads.
