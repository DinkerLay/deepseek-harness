...
`resolveSessionCwd(session)` 与 `executionDirectoryFromEvents(header, events)` 读取归本 Session 所有的 `session/execution-directory` 绑定，未绑定时采用创建 cwd；继承绑定不会改变子 Session 目录。`reserveForDeletion()` 阻止冲突发布并记录已完成删除的代次。`capturePublicationCheck(id)` 将代次校验带过异步源读取，在源被删除后拒绝发布。
