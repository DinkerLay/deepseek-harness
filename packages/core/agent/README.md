...
`AgentHandle` carries `agent`, `dispose()`, and an optional `reserveIdleDisposal()` capability. The registry retains handles returned by `create()` and `resume()` and exposes only the typed `reserveIdleDisposal(id)` attempt, never the teardown handle itself. A claim succeeds only in the loop's exact idle phase with an empty inbox, blocks new input and maintenance, and either follows the ordinary memoized disposal path or is released unused. Directly registered and configuration-created Agents remain unowned by this operation.
