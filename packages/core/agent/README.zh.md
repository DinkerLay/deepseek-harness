...
代码可以通过一个 API 控制 live Agent：创建或恢复、提交跟进或 steering 输入、注入模型上下文、取消工作、等待 idle，并观察活动。它需要与 `dsh-agent-loop` 等 driver 配合，自身不调用模型。创建 interceptor 可以加入可信 setup 与目录策略；idle-disposal 预留会保护精确的非活动 handle，避免并发工作。
