...
Hosts can maintain an ordered persistent list of project directories and the Sessions associated with each directory. Sidebars may hide a Session from grouping without deleting its history, and removing a project never deletes its files or Sessions. Re-adding a directory creates a fresh project; invalid directories remain ungrouped. The feature is invisible to models and requires Session persistence plus storage. Committed Session deletion removes only matching derived membership.
