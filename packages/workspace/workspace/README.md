...
- `ctx.workspaceRegistry.archiveSession(id)` / `archivedSessionIds` — the registry-global archive set layered over workspace accounting. Archive hides a live or persisted Session without moving its `sessionIds` slot. A committed `session-persistence/deleted` removes the id from every Workspace account and the archive set.
