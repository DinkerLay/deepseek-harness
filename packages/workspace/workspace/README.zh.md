...
- `ctx.workspaceRegistry.archiveSession(id)`／`archivedSessionIds`：覆盖在 workspace 记账之上的注册表级全局归档集合。archive 会隐藏 live 或 persisted Session，但不移动其 `sessionIds` 席位。已提交的 `session-persistence/deleted` 会从每个 Workspace account 与 archive set 中移除该 id。
