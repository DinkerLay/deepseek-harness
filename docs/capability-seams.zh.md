...
flowchart LR
  ...
  svc_hostDelegatedAgents["ctx.hostDelegatedAgents<br/>Host-owned delegated execution"]
  ...
  pkg_subagent --> svc_hostDelegatedAgents
...
| `ctx.hostDelegatedAgents` | `service` | [`subagent`](../packages/subagent/subagent) | - | - | - | 受信任的外层 Host 控制器创建显式 Preset 执行，按持久 owner 与声明指纹恢复，对 inbox 投递去重并等待中断或退役；该服务不拥有 Team 策略。 |
