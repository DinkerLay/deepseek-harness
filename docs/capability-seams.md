...
| `ctx.hostDelegatedAgents` | `service` | [`subagent`](../packages/subagent/subagent) | - | - | - | Trusted external Host controllers create explicit-Preset executions, recover by durable owner and declaration digest, deduplicate inbox deliveries, and await interruption or retirement; the service owns no Team policy. |
