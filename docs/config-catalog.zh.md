...
export interface Config {
   ...
   * defaults per-run to the Agent's effective Session execution directory (the
...
需要： `sessionPersistence` · `sessions` · `storageDomain`
...
来源： [`packages/feedback/message-feedback/src/index.ts:43`](../packages/feedback/message-feedback/src/index.ts)
...
- `@deepseek-ai/dsh-session-deletion` — 需要 `sessions` · `sessionPersistence` · `agents` ([`packages/session/session-deletion/src/index.ts`](../packages/session/session-deletion/src/index.ts))
